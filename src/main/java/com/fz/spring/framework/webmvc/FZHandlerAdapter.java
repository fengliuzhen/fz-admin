...
public class FZHandlerAdapter {
    private FZHandlerMapping handlerMapping;
    //记录本Method所有形式参数的位置,为了以后实例参数数组使用
    private Map<String, Integer> methodParasMap = new HashMap<>();

    public FZHandlerAdapter(FZHandlerMapping handlerMapping, Map<String, Integer> methodParasMap) {
    ...
    public FZModelAndView handler(HttpServletRequest request, HttpServletResponse response) throws IllegalAccessException, InstantiationException {
        //1. 从Request中获取所有参数
        ...
        try {
            ...
            if(this.handlerMapping.getMethod().getReturnType()==FZModelAndView.class){
                return (FZModelAndView) result;
            }else
            {
                return null;
            }
